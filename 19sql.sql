...
select * from myemp;
-- Fetch all employees whose salary is 4800, 6000, 9000, 10000
select * from myemp where Salary in(48000,5000,9000);

-- Aggreegation: sum(), Avg(),count(),max(),min():
-- AS is the keyword for alias name
select sum(Salary) as totalsalary from myemp where DEP_ID=60;
select count(Salary),MIN(Salary),MAX(Salary),AVG(Salary) from myemp;
...
alter table student add column dept varchar(20), add column section char(1) after age;
-- DROP we will delete permenetly
...
update student set department='my sql';
...
update student set age = 25 where age is null;

use practicedb;

select * from myemp;
select concat_ws(" ",e.first_name,e.last_name) as emp_fn,e.job_id,
e.dep_id, concat_ws(" ",m.first_name,m.last_name) as mgr_fn
from myemp as e join myemp as m
...
select * from emp_mgr;
-- Q2) Find the count of empoyee reporting to steve king

select count(emp_fn) from emp_mgr where mgr_fn='Steven King';
-- Q3) Which has the most number of reports
select count(emp_fn) as countofemp,mgr_fn from emp_mgr group by mgr_fn order by countofemp desc limit 2;
-- sub quries
-- sub queris with where clause
-- which department has larget employee count? fetch the details of these employees(SQ with where)
select count(emp_id) as countofemp,dep_id
from myemp
group by dep_id
order by countofemp desc
limit 1;

select * from myemp where dep_id =
(
with cte as
(
select count(emp_id) as countofemp,dep_id from myemp group by dep_id order by countofemp desc limit 1
) Select * from cte
);
-- sub queiry with from clause 
-- fetch the count of employees for each dept
-- fetch the mincount, max count and avg CNT OF employees
select min(cntofemp),max(cntofemp),avg(cntofemp) from 
(
select count(emp_id) as cntofemp,dep_id from myemp group by dep_id
) as sq;

show tables;
-- find the avg mrp for each productline 
select * from products;
select min(avgr),max(avgr) from 
(
select avg(mrp) AS avgr from products group by productline
) as sq;


select * from myemp as m where salary > (select avg(salary) from myemp as e where dep_id=m.dep_id);
-- fetch those product details from products table whose mrp is equal or lower than 
-- their specific productline avg mrp
select avg(mrp) as avgr, productline from products group by productline; 
select *from products as p where mrp<=(select avg(mrp) from products as po where productline=p.productline);
-- window functions
-- display avg salary for each employees
select *, avg(salary) over() as overall from myemp;
-- fetch dep avg salary 
select*,avg(salary) over(partition by dep_id) as dep_avg_salary from myemp order by emp_id;
-- fetch all employees whose salary is greater than their dept avg salary 
with cte as
(
select*,avg(salary) over(partition by dep_id) as dep_avg_salary from myemp order by emp_id
)select * from cte where salary>dep_avg_salary;

create table studentsmarks(
stu_id int not null,
marks int);
insert into studentsmarks values
(1,99),(2,98),(3,75),(4,85),(5,88),(6,75),(7,56);

select * from studentsmarks;
select*, rank() over(order by marks desc) as classrank from studentsmarks;
select*, dense_rank() over(order by marks desc) as classdensrank from studentsmarks;
select *, rank() over w as classrank,
dense_rank() over w as classdenserank,
percent_rank() over w 
as percentail
from studentsmarks 
window w as (order by marks desc);

-- rank the sales employee based on sales
select * from sales;
select*, rank() over(order by sale desc) as salerank from sales;
select *, dense_rank() over(order by sale desc) as dense, rank() over(order by sale desc) as salerank from sales;
-- rank sales_employee based on sale amount for each year
select*, dense_rank() over (partition by fiscal_year order by sale ) as rnk 
from sales;
-- rank employees based on salaries use dense_rank
-- rank employees based on salaries for each department
select *, dense_rank() over (order by salary desc) as densrnk from myemp;
select *, dense_rank() over (partition by dep_id order by salary desc) as dprnk from myemp;
select *, row_number() over (order by salary desc) as `S.No` from myemp;
-- using window function fetch the details of highest salaried employee for each department 
select * from 
(
...
-- fetch thye second highest salary person of each
select * from 
(
select*, dense_rank() over (partition by dep_id order by salary desc) as rnk 
from myemp
...
where rnk=2;
-- rank the employees based on the their years of service
select year(curdate());

with cte as
(
select *,year(curdate())-year(hire_date) as yrsofexp from myemp
)
select *, dense_rank()over(order by yrsofexp desc) as rnk from cte;

-- fetch top 10 based on years of exp
with cte as
(
select *,year(curdate())-year(hire_date) as yrsofexp from myemp
)
select *, dense_rank()over(order by yrsofexp desc) as rnk from cte limit 10;
-- fetch top 10 based on years of exp
with cte as
(
select *, timestampdiff(year,hire_date,curdate()) as yrsexp from myemp
), cte2 as
(
select *, dense_rank() over(order by yrsexp desc) as rnk from cte
)
select * from cte2 where rnk<=10;
